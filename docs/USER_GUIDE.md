...
1. 외부 파일 매니저 (Solid Explorer, Material Files, 삼성 **내 파일** 등) 로 `/storage/emulated/0/StS2LauncherMM/Mods/` 경로 진입.
2. 모드 폴더 (예: `MyMod/`) 통째로 그 안에 복사. 폴더 내부에는 `.dll`, 선택적 `.pck`, `<ModId>.json` 매니페스트가 있어야 함. (PC와 동일, 다르지 않음)

  <img src="images/19.jpg" width="600">
  <img src="images/20.jpg" width="600">
  <img src="images/21.jpg" width="600">

3. 게임 실행 시 게임 본체의 **"Load mods?"** 다이얼로그가 뜨면 OK 탭. 한 번 선택하면 그 다음부터 자동 로드.
